...
func load_dialog():
	...
	#print("dialog_script", dialog_script,dialog_script.events)
	for event in dialog_script.events:
		if event.text in Global.dialogic_replace.keys():
			print("dialog_script ", event.text.replace(event.text, Global.dialogic_replace[event.text]))
			event.text = event.text.replace(event.text, Global.dialogic_replace[event.text])
